...
-- Update RLS policies to ensure admins have unrestricted access
...
DO $pol$ BEGIN CREATE POLICY "Admin unrestricted access with enhanced security for others" 
ON public.formulas 
FOR SELECT 
USING (
    NOT is_deleted AND (
        -- ADMINS HAVE UNRESTRICTED ACCESS TO ALL FORMULAS
        has_role(auth.uid(), 'admin'::app_role) OR
        -- Standard formulas: accessible to rd_manager, production_manager
        (security_level = 'standard' AND (
            has_role(auth.uid(), 'rd_manager'::app_role) OR
            has_role(auth.uid(), 'production_manager'::app_role)
        )) OR
        -- Confidential formulas: accessible to rd_manager
        (security_level = 'confidential' AND 
            has_role(auth.uid(), 'rd_manager'::app_role)
        ) OR
        -- Trade secret formulas: use enhanced validation for non-admins
...
-- Add audit logging for admin override access
...
COMMENT ON FUNCTION public.get_accessible_formulas(uuid) IS 'Returns accessible formulas with admin unrestricted access and proper security controls for other roles';