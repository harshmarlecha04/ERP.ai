-- Fix get_material_requirements_by_date_range to use correct recipe field
...
CREATE OR REPLACE FUNCTION public.get_material_requirements_by_date_range(p_start_date date, p_end_date date)
RETURNS TABLE(
...
BEGIN
  ...
  material_requirements AS (
    SELECT 
      ...
      si.batches,
      si.schedule_date,
      -- Use weightKg field from recipe (matches how formulas are stored)
      COALESCE((recipe_item->>'weightKg')::numeric, 0) * si.batches as required_kg
    ...
    WHERE NOT rm.is_archived
      AND COALESCE((recipe_item->>'weightKg')::numeric, 0) > 0
  ),
  aggregated_requirements AS (
    SELECT 
      material_requirements.raw_material_id,
      material_requirements.material_code,
      material_requirements.material_name,
      material_requirements.supplier,
      material_requirements.uom,
      SUM(material_requirements.required_kg) as total_required_kg,
      jsonb_agg(DISTINCT jsonb_build_object(
        'formula_code', material_requirements.formula_code,
        'formula_name', material_requirements.formula_name,
        'batches', material_requirements.batches
      )) as formulas_using,
      jsonb_agg(DISTINCT material_requirements.schedule_date::text) as schedule_dates
    FROM material_requirements
    GROUP BY material_requirements.raw_material_id, material_requirements.material_code, material_requirements.material_name, material_requirements.supplier, material_requirements.uom
  ),
  inventory_status AS (
    SELECT 
      rm.id as raw_material_id,
    ...
    LEFT JOIN raw_material_lots rml ON rml.raw_material_id = rm.id AND rml.quantity > 0
    LEFT JOIN inventory_reservations ir ON ir.lot_id = rml.id
    GROUP BY rm.id
  ),
  ...
  SELECT 
    ...
    ar.total_required_kg - COALESCE(inv.current_inventory_kg - inv.reserved_kg, 0) - COALESCE(po.on_order_kg, 0) as net_after_orders_kg,
    ar.formulas_using,
    ar.schedule_dates,
