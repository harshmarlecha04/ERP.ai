-- Update validate_formula_access_secure to allow production managers access to trade secrets for production operations
...
BEGIN
    ...
    IF formula_security_level = 'trade_secret' OR formula_classification = 'trade_secret' THEN
        ...
        IF has_role(_user_id, 'production_manager'::app_role) AND _access_type = 'view' THEN
            -- Log the production access
            PERFORM log_formula_access(
                _user_id, 
                _formula_id, 
                'production_access',
                jsonb_build_object(
                    'access_type', 'production_view',
                    'classification', formula_classification,
                    'security_level', formula_security_level,
                    'justification', 'Production manager accessing for scheduled production operations'
                )
            );
            RETURN true;
        END IF;
        
        -- For other users (including admins), check for active approved session
        SELECT EXISTS (
        ...
        IF has_role(_user_id, 'admin'::app_role) THEN
            ...
            PERFORM log_formula_access(
                ...
                jsonb_build_object(
                    ...
                    'classification', formula_classification,
                    'security_level', formula_security_level,
                    'justification', 'Admin override for trade secret access'
                )
            );
            RETURN true;
        END IF;
