...
BEGIN
  ...
  WITH schedule_items AS (
    ...
    JOIN production_schedule ps ON ps.id = psi.schedule_id
    JOIN formulas f ON f.id = psi.formula_id
    WHERE ps.scheduled_date BETWEEN p_start_date AND p_end_date
  ...
  aggregated_requirements AS (
    SELECT 
      ...
      jsonb_agg(DISTINCT scheduled_date::text) as schedule_dates
  ...
  SELECT 
    ...
    -- FIXED: Ensure net_after_orders_kg is always >= 0 (shortage amount, not negative surplus)
