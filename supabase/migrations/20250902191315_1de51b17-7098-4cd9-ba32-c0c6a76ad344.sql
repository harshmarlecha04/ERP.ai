-- Fix User Role Information Security Issue
...
SELECT 
  ...
  EXISTS(SELECT 1 FROM public.user_roles WHERE user_id = auth.uid() AND role = 'formulation_scientist') as is_formulation_scientist,
