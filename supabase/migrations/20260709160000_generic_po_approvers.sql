-- PO approvers are now the company's admins (dynamic), not a hardcoded email list.
...
AS $function$
DECLARE
  _approvers text[] := COALESCE((SELECT array_agg(p.email) FROM public.profiles p JOIN public.user_roles r ON r.user_id = p.id WHERE r.role = 'admin' AND p.email IS NOT NULL), ARRAY['mfg@pharmvista.com','bizops@pharmvista.com','licensing@pharmvista.com','sales@pharmvista.com']);
  _header record;
  _customer_name text;
  _approver_id uuid;
  _email text;
...
CREATE OR REPLACE FUNCTION public.decide_po_approval(_order_id uuid, _decision text, _rejection_reason text DEFAULT NULL::text, _line_formulas jsonb DEFAULT '{}'::jsonb)
 RETURNS void
 LANGUAGE plpgsql
 SECURITY DEFINER
 SET search_path TO 'public'
