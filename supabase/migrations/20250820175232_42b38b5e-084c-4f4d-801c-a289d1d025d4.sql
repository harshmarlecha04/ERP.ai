-- Fix the upsert_raw_material_with_lots function to handle existing lot IDs properly
...
BEGIN
  ...
  -- Fast CTE-based lot sync - handle both new and existing lots
  ...
  -- Upsert incoming lots using both primary key (id) and unique constraint conflicts
  upserted_lots AS (
  ...
  -- Only delete lots that are no longer in the payload (based on id)
