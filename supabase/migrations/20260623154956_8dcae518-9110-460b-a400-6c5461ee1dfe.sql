...
ALTER TABLE public.launch_milestone_tasks ENABLE ROW LEVEL SECURITY;
CREATE POLICY "auth read mtasks" ON public.launch_milestone_tasks FOR SELECT TO authenticated USING (true);
CREATE POLICY "auth write mtasks" ON public.launch_milestone_tasks FOR INSERT TO authenticated WITH CHECK (auth.uid() IS NOT NULL);
CREATE POLICY "auth delete mtasks" ON public.launch_milestone_tasks FOR DELETE TO authenticated USING (auth.uid() IS NOT NULL);
...
ALTER TABLE public.launch_charters ENABLE ROW LEVEL SECURITY;
CREATE POLICY "auth read charters" ON public.launch_charters FOR SELECT TO authenticated USING (true);
CREATE POLICY "auth insert charters" ON public.launch_charters FOR INSERT TO authenticated WITH CHECK (auth.uid() IS NOT NULL);
CREATE POLICY "auth update charters" ON public.launch_charters FOR UPDATE TO authenticated USING (auth.uid() IS NOT NULL) WITH CHECK (auth.uid() IS NOT NULL);
CREATE POLICY "auth delete charters" ON public.launch_charters FOR DELETE TO authenticated USING (auth.uid() IS NOT NULL);
...
ALTER TABLE public.launch_stakeholders ENABLE ROW LEVEL SECURITY;
CREATE POLICY "auth read stk" ON public.launch_stakeholders FOR SELECT TO authenticated USING (true);
CREATE POLICY "auth insert stk" ON public.launch_stakeholders FOR INSERT TO authenticated WITH CHECK (auth.uid() IS NOT NULL);
CREATE POLICY "auth update stk" ON public.launch_stakeholders FOR UPDATE TO authenticated USING (auth.uid() IS NOT NULL) WITH CHECK (auth.uid() IS NOT NULL);
CREATE POLICY "auth delete stk" ON public.launch_stakeholders FOR DELETE TO authenticated USING (auth.uid() IS NOT NULL);
CREATE INDEX idx_launch_stk_project ON public.launch_stakeholders(project_id);
...
CREATE POLICY "auth read risks" ON public.launch_risks FOR SELECT TO authenticated USING (true);
CREATE POLICY "auth insert risks" ON public.launch_risks FOR INSERT TO authenticated WITH CHECK (auth.uid() IS NOT NULL);
CREATE POLICY "auth update risks" ON public.launch_risks FOR UPDATE TO authenticated USING (auth.uid() IS NOT NULL) WITH CHECK (auth.uid() IS NOT NULL);
CREATE POLICY "auth delete risks" ON public.launch_risks FOR DELETE TO authenticated USING (auth.uid() IS NOT NULL);
CREATE INDEX idx_launch_risks_project ON public.launch_risks(project_id);
...
CREATE POLICY "auth read snap" ON public.launch_status_snapshots FOR SELECT TO authenticated USING (true);
CREATE POLICY "auth insert snap" ON public.launch_status_snapshots FOR INSERT TO authenticated WITH CHECK (auth.uid() IS NOT NULL);
CREATE POLICY "auth update snap" ON public.launch_status_snapshots FOR UPDATE TO authenticated USING (auth.uid() IS NOT NULL) WITH CHECK (auth.uid() IS NOT NULL);
CREATE POLICY "auth delete snap" ON public.launch_status_snapshots FOR DELETE TO authenticated USING (public.has_role(auth.uid(),'admin'));
CREATE INDEX idx_launch_snap_project ON public.launch_status_snapshots(project_id, captured_at DESC);
