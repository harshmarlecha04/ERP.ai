...
BEGIN
    ...
    -- Allow access during business hours for all authenticated users
    IF is_business_time AND _user_id IS NOT NULL THEN
        RETURN true;
    END IF;
    
    -- Outside business hours, restrict to admins and HR managers
    RETURN (
...
BEGIN
    ...
    -- During business hours, all authenticated users can view profiles
    IF is_business_time AND _viewer_id IS NOT NULL THEN
        -- Log the access
        INSERT INTO public.profile_access_audit (
            viewer_id, profile_id, access_type, access_reason, risk_level
        ) VALUES (
            _viewer_id, _profile_id, 'business_hours_view', 'business_hours_policy', 'low'
        );
        RETURN true;
    END IF;
    
    -- Outside business hours, use existing restrictions
    IF _viewer_id = _profile_id THEN
        RETURN true; -- Own profile
    ELSIF has_role(_viewer_id, 'admin') THEN
