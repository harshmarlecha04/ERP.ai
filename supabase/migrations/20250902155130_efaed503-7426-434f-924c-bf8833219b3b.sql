...
-- First check and drop all existing policies for purchase_orders

DO $$
BEGIN
    -- Drop all existing policies for purchase_orders
    ...
    EXECUTE 'CREATE POLICY "All authenticated users can view purchase orders"
    ON public.purchase_orders
    FOR SELECT
    TO authenticated
    USING (auth.uid() IS NOT NULL)';
    
    EXECUTE 'CREATE POLICY "All authenticated users can create purchase orders"
    ON public.purchase_orders
    FOR INSERT
    TO authenticated
    WITH CHECK (auth.uid() IS NOT NULL)';
    
    EXECUTE 'CREATE POLICY "All authenticated users can update purchase orders"
    ON public.purchase_orders
    FOR UPDATE
    TO authenticated
    USING (auth.uid() IS NOT NULL)
    ...
    EXECUTE 'CREATE POLICY "Only admins can delete purchase orders"
    ON public.purchase_orders
    FOR DELETE
    TO authenticated
