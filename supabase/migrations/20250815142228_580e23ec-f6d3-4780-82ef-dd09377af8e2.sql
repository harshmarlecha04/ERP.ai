...
-- Ensure permissions table exists (was created outside the migration chain originally)
CREATE TABLE IF NOT EXISTS public.formula_access_permissions (
  id uuid PRIMARY KEY DEFAULT gen_random_uuid(),
  user_id uuid NOT NULL,
  formula_id uuid,
...
DO $rls$ BEGIN ALTER TABLE public.formula_access_permissions ENABLE ROW LEVEL SECURITY; EXCEPTION WHEN wrong_object_type OR feature_not_supported THEN NULL; END $rls$;
