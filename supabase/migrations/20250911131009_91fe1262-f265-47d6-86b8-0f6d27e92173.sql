-- CRITICAL SECURITY FIX: Phase 2 (Safe approach)
-- Temporarily disable problematic trigger, apply security fixes, then re-enable

-- 1. Temporarily disable the problematic trigger to allow security updates
DROP TRIGGER IF EXISTS audit_formula_access_trigger ON public.formulas;

-- 2. Upgrade all formulas with recipe data to trade secret status
...
-- 3. Update RLS policy to use the new strict validation
...
-- 5. Add function to check trade secret access status
