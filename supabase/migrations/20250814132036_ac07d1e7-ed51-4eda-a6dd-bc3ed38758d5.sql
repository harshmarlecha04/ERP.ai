...
BEGIN
  ...
  -- Check for duplicate code (case insensitive, fast lookup)
  ...
  IF jsonb_array_length(p_lots) > 0 THEN
    ...
    SELECT array_agg(
      jsonb_build_object(
        ...
        'lot_number', NULLIF(trim(lot_data->>'lot_number'), ''),
        'quantity', COALESCE((lot_data->>'quantity')::numeric, 0),
        'cost', COALESCE((lot_data->>'cost')::numeric, 0),
        'expiry_date', NULLIF(lot_data->>'expiry_date', '')::date,
        'coa_link', NULLIF(trim(lot_data->>'coa_link'), '')
      )
    ) INTO v_lot_insert_data
    ...
    SELECT 
      (lot_obj->>'raw_material_id')::uuid,
      lot_obj->>'lot_number',
      (lot_obj->>'quantity')::numeric,
      (lot_obj->>'cost')::numeric,
      (lot_obj->>'expiry_date')::date,
      lot_obj->>'coa_link'
    FROM unnest(v_lot_insert_data) AS lot_obj;
