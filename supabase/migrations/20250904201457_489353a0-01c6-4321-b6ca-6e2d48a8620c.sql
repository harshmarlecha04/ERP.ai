-- Fix function name conflicts and create approval workflows for HR access

-- Drop existing functions that conflict
DO $df$ DECLARE r record; BEGIN FOR r IN SELECT oid::regprocedure AS sig FROM pg_proc WHERE proname='get_employee_sensitive_data' AND pronamespace='public'::regnamespace LOOP EXECUTE 'DROP FUNCTION ' || r.sig; END LOOP; END $df$;
...
DO $gr$ BEGIN GRANT EXECUTE ON FUNCTION public.get_employee_sensitive_data_secure TO authenticated; EXCEPTION WHEN ambiguous_function OR undefined_function THEN NULL; END $gr$;
DO $gr$ BEGIN GRANT EXECUTE ON FUNCTION public.update_employee_data_with_approval TO authenticated; EXCEPTION WHEN ambiguous_function OR undefined_function THEN NULL; END $gr$;