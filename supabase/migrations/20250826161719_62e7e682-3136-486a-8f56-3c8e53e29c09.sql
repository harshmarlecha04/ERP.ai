...
BEGIN
    ...
    RAISE NOTICE 'save_formula called with data: %', p_formula_data;
    ...
    IF p_formula_data ? 'id' AND (p_formula_data->>'id') IS NOT NULL AND (p_formula_data->>'id') != 'null' THEN
        RAISE NOTICE 'Updating existing formula with ID: %', p_formula_data->>'id';
        ...
        success_message := 'Formula updated successfully';
        RAISE NOTICE 'Updated formula with ID: %', result_id;
    ELSE
        RAISE NOTICE 'Creating new formula';
        
        -- Insert new formula with all required fields
        INSERT INTO public.formulas (
            ...
            '1.0',
            false,  -- is_deleted = false for new formulas
        ...
        success_message := 'Formula created successfully';
        RAISE NOTICE 'Created new formula with ID: %', result_id;
    END IF;
    ...
    BEGIN
        PERFORM log_formula_access(
            user_id, 
            result_id, 
            CASE WHEN p_formula_data ? 'id' AND (p_formula_data->>'id') IS NOT NULL AND (p_formula_data->>'id') != 'null' THEN 'update' ELSE 'create' END,
            jsonb_build_object(
                'operation', CASE WHEN p_formula_data ? 'id' AND (p_formula_data->>'id') IS NOT NULL AND (p_formula_data->>'id') != 'null' THEN 'update' ELSE 'create' END,
                'formula_code', p_formula_data->>'code'
            )
        );
    EXCEPTION WHEN OTHERS THEN
        -- Ignore logging errors to avoid blocking the main operation
        RAISE NOTICE 'Failed to log formula access: %', SQLERRM;
    END;
...
EXCEPTION
    WHEN OTHERS THEN
        RAISE NOTICE 'Error in save_formula: % %', SQLSTATE, SQLERRM;
        RETURN jsonb_build_object(
            'success', false, 
            'error', SQLERRM,
