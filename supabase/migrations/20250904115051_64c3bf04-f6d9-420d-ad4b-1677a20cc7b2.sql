...
CREATE POLICY "Employees can update limited personal info" ON public.employee_sensitive_data
FOR UPDATE USING (id = auth.uid()) 
WITH CHECK (
    id = auth.uid() AND
    -- Prevent employees from changing sensitive fields like salary_band, security_clearance
    OLD.salary_band IS NOT DISTINCT FROM NEW.salary_band AND
    ...
    OLD.hire_date IS NOT DISTINCT FROM NEW.hire_date
);
