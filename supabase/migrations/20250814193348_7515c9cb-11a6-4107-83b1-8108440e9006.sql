...
-- Add case-insensitive code column to raw_materials if it doesn't exist
DO $$
BEGIN
  IF NOT EXISTS (SELECT 1 FROM information_schema.columns WHERE table_name = 'raw_materials' AND column_name = 'code_ci') THEN
    ALTER TABLE public.raw_materials 
      ADD COLUMN code_ci citext GENERATED ALWAYS AS (code::citext) STORED;
  END IF;
END $$;

-- Add unique constraint on case-insensitive code if it doesn't exist
DO $$
BEGIN
  IF NOT EXISTS (SELECT 1 FROM pg_constraint WHERE conname = 'raw_materials_code_ci_key') THEN
    ALTER TABLE public.raw_materials 
      ADD CONSTRAINT raw_materials_code_ci_key UNIQUE (code_ci);
  END IF;
END $$;
...
DO $$
BEGIN
  -- Check if unit_of_measure exists and rename to uom
  IF EXISTS (SELECT 1 FROM information_schema.columns WHERE table_name = 'raw_materials' AND column_name = 'unit_of_measure') THEN
    ALTER TABLE public.raw_materials RENAME COLUMN unit_of_measure TO uom;
  END IF;
  
  -- Check if expiry_date exists and rename to expires_on in lots table
  IF EXISTS (SELECT 1 FROM information_schema.columns WHERE table_name = 'raw_material_lots' AND column_name = 'expiry_date') THEN
    ALTER TABLE public.raw_material_lots RENAME COLUMN expiry_date TO expires_on;
  END IF;
EXCEPTION
  WHEN duplicate_column THEN
    -- Column already renamed, continue
    NULL;
END $$;
