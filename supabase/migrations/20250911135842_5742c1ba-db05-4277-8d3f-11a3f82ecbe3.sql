-- CRITICAL SECURITY FIX: Enhanced Formula Protection Against Industrial Espionage
-- First drop existing function that conflicts with new signature
...
BEGIN
    ...
    SELECT 
        ...
        f.total_pieces,
        -- Redact sensitive procedure text for trade secrets
        CASE 
            WHEN f.security_level = 'trade_secret' THEN
                CASE 
                    ...
                    ELSE '[TRADE SECRET - ACCESS RESTRICTED - CONTACT ADMINISTRATOR]'
                END
        ...
        CASE 
            WHEN f.security_level = 'trade_secret' THEN
                CASE 
                    WHEN public.validate_trade_secret_access_enhanced(_user_id, f.id) THEN f.notes
                    ELSE '[REDACTED - TRADE SECRET]'
                END
            ELSE f.notes
        END as notes,
        f.status,
    ...
    AND (
        ...
        OR
        -- Trade secrets: show basic info to admins, redacted sensitive data
...
AS $$
BEGIN
    -- Only admins can activate emergency lockdown
    IF NOT has_role(auth.uid(), 'admin'::app_role) THEN
        RAISE EXCEPTION 'Access denied: Only administrators can activate emergency lockdown';
    END IF;
    
    -- Set emergency lockdown flag
    INSERT INTO public.security_config (config_key, config_value)
    VALUES ('emergency_lockdown', '{"enabled": true}'::jsonb)
    ...
    INSERT INTO public.security_alerts (alert_type, severity, details)
    VALUES (
        'emergency_formula_lockdown_activated',
        'critical',
        jsonb_build_object(
            'activated_by', auth.uid(),
            'timestamp', now(),
            'reason', 'Emergency lockdown activated - all formula access suspended'
        )
    );
END;
$$;
...
AS $$
BEGIN
    -- Only admins can disable emergency lockdown
    IF NOT has_role(auth.uid(), 'admin'::app_role) THEN
        RAISE EXCEPTION 'Access denied: Only administrators can disable emergency lockdown';
    END IF;
    
    -- Disable emergency lockdown flag
    ...
    VALUES (
        ...
        jsonb_build_object(
            ...
            'timestamp', now(),
            'reason', 'Emergency lockdown disabled - normal access restored'
        )
    );
END;
$$;
...
COMMENT ON FUNCTION public.get_accessible_formulas(uuid) IS 'SECURITY-CRITICAL: Enhanced formula access with data redaction, comprehensive logging, and suspicious activity detection for trade secret protection.';
COMMENT ON FUNCTION public.validate_trade_secret_access_enhanced(uuid, uuid) IS 'SECURITY-CRITICAL: Military-grade validation for trade secret access with business hours, role validation, and threat detection.';
...
VALUES (
    ...
    jsonb_build_object(
        ...
        'features', jsonb_build_array(
            ...
            'Enhanced trade secret validation',
