-- Fix the RPC function that's causing the "is_deleted" field error during formula creation
-- The issue is likely in a trigger or RPC function that assumes is_deleted exists on new records

-- First, let's check if there are any triggers on the formulas table that might be causing this
