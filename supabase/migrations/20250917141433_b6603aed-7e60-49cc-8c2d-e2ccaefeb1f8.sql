...
BEGIN
  ...
  BEGIN
    ...
    LOOP
      -- Extract ingredient information with unit conversion support
      ...
      -- Get total available inventory for this material (with row lock)
      ...
      -- Assign lots using FIFO with enhanced sorting
      ...
      FOR v_lot IN 
        ...
        AND l.quantity > 0
        -- Enhanced FIFO: receiving_date first, then created_at, then ID for consistency
        ORDER BY 
...
DO $pol$ BEGIN CREATE POLICY "Authenticated users can manage usage sessions" 
