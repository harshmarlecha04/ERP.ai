...
DECLARE
    ...
    v_similarity_threshold numeric := 0.3; -- Minimum similarity score (0-1)
BEGIN
    -- Get formula recipe
    ...
    LOOP
        ...
        IF v_material_id IS NULL THEN
            SELECT id, name INTO v_material_id, v_material_name
            FROM public.raw_materials
            WHERE similarity(LOWER(name), LOWER(v_ingredient_name)) > v_similarity_threshold
            ORDER BY similarity(LOWER(name), LOWER(v_ingredient_name)) DESC
            LIMIT 1;
        END IF;
        ...
        v_ingredient_details := v_ingredient_details || jsonb_build_array(
            jsonb_build_object(
                'ingredient_id', COALESCE(v_material_id::text, ''),
                'ingredient_name', v_ingredient_name,
                'matched_material_name', COALESCE(v_material_name, ''),
                'available_kg', v_available_qty,
                'required_per_batch_kg', v_required_qty,
